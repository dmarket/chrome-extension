...
import { dmExtDmarketOfficialUrl, dmExtDmarketSupportOfficialUrl } from '@myth/dm-ext-shared-constants';
...
(async function () {
  try {
    await domLoaded;
    const dmSiteElements = $$(`#search a[href^="${dmExtDmarketOfficialUrl}"]`);
    const dmSupportLinkElements = $$(`#search a[href^="${dmExtDmarketSupportOfficialUrl}"]`);
    const dmLinkElements = [...dmSiteElements, ...dmSupportLinkElements];
    if (!dmLinkElements.length && !dmSupportLinkElements) return;
    ...
    for (const index of [...dmLinkElements.keys()].reverse()) {
      ...
      const isArrowLink = dmLinkElement?.querySelectorAll(':scope > .DYjfCf')?.length > 0;
      ...
      if (dmLinkInfoElement) {
        const hasTransformedParent = dmLinkInfoElement.closest('.V9tjod');
        if (hasTransformedParent) {
          const transformStyle = window.getComputedStyle(hasTransformedParent)?.transform ?? '';
          if (transformStyle && transformStyle.includes('-1')) {
            widget.style.transform = 'scaleY(-1)';
          }
        }
      }
      if (dmLinkInfoElement) {
        if (isTitleLink || isImageSearchPageSelected || isArrowLink) {
